...
#outfolder = root_dir / "outputs" / "spatial-analysis-figs" / "wrf-clouds-sci" / "low-clouds-and-fog" # Where to save figures to 
outfolder = root_dir / "outputs" / "spatial-analysis-figs" / "wrf-clouds-with-wind-vectors"
...
def draw_basemap(mapcrs):
    ...
    # tick params
    ax.tick_params(direction='out', labelsize=12, length=5, pad=2, color='black')    
    # Map features
...
for t in np.arange(0,timesteps,1):#(0,73,1): # loop over times in file
    ...
    # so it should be 30...
    cloudFrac=wrf.g_cloudfrac.get_cloudfrac(ncfile,timeidx=t,vert_type='height_msl',
                                            low_thresh=low_cloud,
                                            mid_thresh=mid_cloud,
                                            high_thresh=high_cloud) # this is all three layers, we subselect later.
    
    cloudFracFog=wrf.g_cloudfrac.get_cloudfrac(ncfile,timeidx=t,vert_type='height_msl',
...
#                     colors='k',linewidths=0.5,transform=crs.PlateCarree()) # terrain elevation contours
        ...
        if l==3:
            ...
            cloudContourLine=ax_cld.contour(to_np(lons), to_np(lats), to_np(cloudFracFog[0,:,:]), cldLvls,
                    colors='k',linewidths=0.5,transform=crs.PlateCarree())
